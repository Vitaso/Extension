...
const ViteNS_OffCCode = buffer.Buffer.from(
  ...
  , 'hex').toString('base64')
const ViteNS_contractAddress = 'vite_1077691249858a325a4387fa77a203e494d08167fa6234bc01'
...
document.addEventListener('DOMContentLoaded', function (event) {
  chrome.storage.local.get('wallet_mnemo', function (data) {
    if (typeof data.wallet_mnemo !== 'undefined') {
      ...
      walletObject = $vite_vitejs.wallet.getWallet(data.wallet_mnemo).deriveAddress(0)
      refreshWallet()
  ...
  document.getElementById("walletScreen_sendMenu_send").onclick = async function () {
    if (document.getElementById("walletScreen_sendMenu_address").value.endsWith('.vite')) {
      const call = $vite_vitejs.abi.encodeFunctionCall(ViteNS_Abi, [ document.getElementById("walletScreen_sendMenu_address").value.replace('.vite', '') ], "resolveAddress")
      ...
      const decoded = $vite_vitejs.abi.decodeParameters(
          ViteNS_Abi.find(e => e.name === "resolveAddress").outputs.map(e => e.type),
          buffer.Buffer.from(result, "base64").toString("hex")
      )

      if (decoded[0] === 'vite_0000000000000000000000000000000000000000a4f3a0cb58') return bulmaToast.toast({
      ...
      document.getElementById("walletScreen_sendMenu_address").value = decoded[0]
    }

    const sendBlock = $vite_vitejs.accountBlock.createAccountBlock('send', {
      address: walletObject.address,
      amount: (parseFloat(document.getElementById("walletScreen_sendMenu_amount").value) * parseFloat(`1e${document.getElementById("walletScreen_sendMenu_tokens_" + document.getElementById("walletScreen_sendMenu_tokens").options[document.getElementById("walletScreen_sendMenu_tokens").selectedIndex].innerHTML).getAttribute("decimals")}`)).toString(),
      tokenId: document.getElementById("walletScreen_sendMenu_tokens_" + document.getElementById("walletScreen_sendMenu_tokens").options[document.getElementById("walletScreen_sendMenu_tokens").selectedIndex].innerHTML).getAttribute("tokenid"),
      toAddress: document.getElementById("walletScreen_sendMenu_address").value
    }).setProvider(viteNode).setPrivateKey(walletObject.privateKey)

    const [ quota, difficulty ] = await Promise.all([
      viteNode.request("contract_getQuotaByAccount", walletObject.address),
      sendBlock.autoSetPreviousAccountBlock()
      .then(() => viteNode.request("ledger_getPoWDifficulty", {
  ...
  document.getElementById("walletScreen_controlTab_send").onclick = function () {
    hide("walletScreen_balances")
    show("walletScreen_sendMenu")
    hide("walletScreen_options")
  }

  document.getElementById("walletScreen_controlTab_balances").onclick = function () {
    show("walletScreen_balances")
    hide("walletScreen_sendMenu")
    hide("walletScreen_options")
  }

  document.getElementById("walletScreen_controlTab_options").onclick = function () {
    hide("walletScreen_balances")
    hide("walletScreen_sendMenu")
    show("walletScreen_options")
  }

  document.getElementById("walletScreen_options_deleteWallet").onclick = function () {
  ...
  document.getElementById("setupWizard_import_import").onclick = function () {
    if (!$vite_vitejs.wallet.validateMnemonics(document.getElementById('setupWizard_import_mnemonics'))) return bulmaToast.toast({
...
const refreshWallet = () => {
  document.getElementById("walletScreen_walletAddr").innerHTML = '<span class="tag is-rounded is-primary">Copy</span> ' + walletObject.address.split('', 18).join('') + '...'

  document.getElementById("walletScreen_balances").innerHTML = ""

  viteNode.request('ledger_getAccountInfoByAddress', walletObject.address).then(data => {
    document.getElementById('walletScreen_walletBal').innerHTML = `${balanceToReadable(data.balanceInfoMap?.[$vite_vitejs.constant.Vite_TokenId]?.balance ?? "0", "18")} VITE <img width="20" height="20" style="vertical-align:middle;" src="./dist/vite/vite.webp" draggable=false>`
    
    lastBalances = data.balanceInfoMap

    Object.entries(data.balanceInfoMap).forEach((token, index) => {
      if (token[1].tokenInfo.tokenId === $vite_vitejs.constant.Vite_TokenId) return
      if (token[1].balance == 0) return

      document.getElementById("walletScreen_balances").innerHTML += `
          <li style="background-color: rgb(27, 27, 27) !important;">
            <span class="icon"><i><img id="walletScreen_balances_${index}" width="100" height="100" style="vertical-align:middle;" src="https://vite-api.thomiz.dev/crypto-info/tokens/${token[1].tokenInfo.tokenId}/icon.png" draggable=false></i></span>
            <span>${token[1].tokenInfo.tokenName}</span>
            <span style="float: right;">${parseInt(token[1].balance) / parseFloat("1e+"+token[1].tokenInfo.decimals)} ${token[1].tokenInfo.tokenSymbol}</span>
          </li>
          <div class="margin-10"></div>
      `

      document.getElementById("walletScreen_balances_" + index).onerror = function () {
        document.getElementById("walletScreen_balances_" + index).onerror = null
        document.getElementById("walletScreen_balances_" + index).src = './dist/vite/vite.webp'
      }
    })
